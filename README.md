...
| Page | What it does |
| --- | --- |
| **Landing** (`/`) | Built as a magazine rather than a catalogue. A hero told as a story: every image from Settings → Storefront becomes a frame with its own progress segment, advancing itself, tappable left and right. Categories sit under it as story rings — a strip, not half a screen, because the page's job there is to get someone into a room rather than to dwell on the door. Featured pieces are a deck of cards you throw sideways, one at a time, on the one dark panel in an ivory page. What just landed is a wall of unequal tiles whose last tile takes up whatever the row has left over, so it never ends mid-row. The shop's own words sit in the middle, beside a picture or a clip it chose. Then best sellers and the way into the shop. Every heading, every line under one, and every button on the page is written in the dashboard. |
